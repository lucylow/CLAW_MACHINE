...
exports.buildMixedPrompt = buildMixedPrompt;
exports.buildReasoningPrompt = buildReasoningPrompt;
const utils_js_1 = require("./utils.js");
function buildImagePrompt(asset, context) {
    const label = (0, utils_js_1.buildAssetLabel)(asset);
    return [
        ...
        `Context: ${(0, utils_js_1.summarizeJson)(context.context || {}, 400)}`,
    ].join("\n");
}
function buildAudioPrompt(asset, context) {
    const label = (0, utils_js_1.buildAssetLabel)(asset);
    return [
...
function buildMixedPrompt(context) {
    const assetLines = context.assets.map((asset) => `- ${(0, utils_js_1.buildAssetLabel)(asset)}`).join("\n");
    return [
        ...
        assetLines,
        "",
        `Context: ${(0, utils_js_1.summarizeJson)(context.context || {}, 500)}`,
    ].join("\n");
}
